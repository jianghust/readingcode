...
/*
Nginx中定义了基本的数据结构ngx_connection_t来表示连接，这个连接表示是客户端主动发起的、Nginx服务器被动接受的TCP连接，简单称
其为被动连接。同时，在有些请求的处理过程中，Nginx会试图主动向其他上游服务器建立连接，并以此连接与上游服务器通信，因此，这样的
连接与ngx_connection_t又是不同的，Nginx定义了ngx_peer_connection_t结构体来表示主动连接，当然，ngx_peer_connection_t主动连接是
以ngx_connection-t结构体为基础实现的。这两种连接都不可以随意创建，必须从连接池中获取
*/
...
struct ngx_peer_connection_s {
    ...
    ngx_uint_t                       tries;
	//向后端服务器发起连接的时间ngx_http_upstream_init_request
	...
	//例如rr算法,对应结构ngx_http_upstream_rr_peer_data_t，创建空间在ngx_http_upstream_create_round_robin_peer
    void                            *data;
	...
	//本机地址信息 //proxy_bind  fastcgi_bind 设置的本地IP端口地址，有可能设备有好几个eth，只用其中一个
    int                              type;
	//套接字的接收缓冲区大小
    int                              rcvbuf;
